...
namespace tinyobj {
    ...
    static inline void parsedouble2(
        double& x, double& y,
        const char*& token)
    {
        x = parsedouble(token);
    ...
    static inline void parsedouble3(
        ...
        std::istream& inStream)
    {
        ...
        while (inStream.peek() != -1) {
            ...
            if (token[0] == 'N' && token[1] == 'i' && isSpace((token[2]))) {
                token += 2;
                material.ior = parsedouble(token);
                continue;
            }
            ...
            if (token[0] == 'N' && token[1] == 's' && isSpace(token[2])) {
                token += 2;
                material.shininess = parsedouble(token);
                continue;
            }
            ...
            if ((token[0] == 'd' && isSpace(token[1]))) {
                token += 1;
                material.dissolve = parsedouble(token);
                continue;
            }
